...
class DraggableBottomSheet extends StatefulWidget {
  ...
  /// Color of the modal barrier. Default black54
...
class DraggableBottomSheetState extends State<DraggableBottomSheet> {
  ...
  Widget _body() {
    return Stack(
      children: [
        ...
        // barrier
        if (_currentExtent.roundToDouble() > widget.minExtent + 1)
  ...
  Widget _sheet() {
    return GestureDetector(
      onVerticalDragEnd: (details) {
        if (_currentExtent <= (screenHeight(context) / 4)) {
          setState(() {
            _currentExtent = 135;
            widget.onDragging(_currentExtent);
          });
        } else if (_currentExtent > (screenHeight(context) / 4)) {
          setState(() {
  ...
  void _onVerticalDragUpdate(DragUpdateDetails details) {
    ...
    final newExtent = _currentExtent - details.delta.dy;

    if (newExtent <= widget.maxExtent) {
      setState(() {
        _currentExtent = newExtent;
        widget.onDragging(_currentExtent);
      });
    } else if (newExtent > widget.maxExtent) {
      setState(() {
        _currentExtent = widget.maxExtent;
        widget.onDragging(_currentExtent);
      });
